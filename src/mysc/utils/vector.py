...
class ScalePointR:
    ...
    def rotate(self) -> Self:
        ...
        return ScalePointR(
            self.y, self.x,
            EnumDirection.HORIZONTAL if self.r == EnumDirection.VERTICAL else EnumDirection.VERTICAL
        )
    ...
    def with_direction(self, direction: EnumDirection) -> Self:
        """
            按方向输出
        :param direction:
        :return:
        """
        return self if direction == self.r else self.rotate()
...
class Coordinate:
    ...
    def __repr__(self):
        return f'w/h ({self.width:>5} / {self.height:<5}) | {"Up" if self.current_direction is EnumDirection.VERTICAL else "Right"}'
    ...
    def to_scale_point_r(self, x: int, y: int) -> ScalePointR:
        return ScalePointR(x / self.width, y / self.height, self.current_direction)
    ...
    @property
    def current_direction(self) -> EnumDirection:
        return EnumDirection.VERTICAL if self.height >= self.width else EnumDirection.HORIZONTAL
    ...
    def get_max_coordinate(self, max_width: int = 0, max_height: int = 0) -> Self:
        """
            获取限制下最大坐标系
        :param max_width:
        :param max_height:
    ...
    def fix_height(self, raw_coordinate: Self) -> Self:
        """
    ...
    def fix_width(self, raw_coordinate: Self) -> Self:
        """
    ...
    def fit_scrcpy_video(self) -> Self:
        """
    ...
    @property
    def horizontal_wh(self) -> Self:
        """
            返回水平 width height
        :return:
        """
        return Coordinate(self.max_size, self.min_size)

    @property
    def vertical_wh(self) -> Self:
        """
            返回垂直 width height
        :return:
        """
        return Coordinate(self.min_size, self.max_size)
